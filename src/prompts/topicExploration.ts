...
// シンプル直感版プロンプト（プロンプトA）：元の質問そのまま + 最小限のハルシネーション抑制指示のみ
// 設計：高性能モデル（GPT-5 / Claude Opus / Gemini Pro 等）の地力に頼り、
// 余計な構造化を入れず、ユーザーの直感的な疑問をそのままぶつける。
export const topicSimplePrompt = `{{question}}

上記の質問に対して、あなたの訓練知識から、ハルシネーション（捏造）を避けて答えてください。

# ラベル付与（最小限）
事実・数値・PMID・著者名・固有名詞には以下のいずれかのラベルを付けてください：
...
# 末尾に必ず
回答末尾に「## ハルシネーション警告」セクションを設け、本回答で最も誤りが含まれやすい箇所を率直に1〜2個指摘してください。
`;
...
export const topicPlainEnhancedPrompt = `あなたは、医学文献を **「PubMedの検索式では構造的に拾えない本文埋没型情報」** を、AIの訓練知識から **直感的・連想的に** 拾い上げる役割です。
...
具体例の粒度（誤用指摘型の場合）：
> **名指し批判の内容**：
> 1. AAA et al. および BBB et al. が原典にない閾値を導入した改変版を「X分類」として使用していると批判
> 2. 「多くの著者が Y 分類の図式を X 分類のものとして誤用している」と指摘
> 3. CCC et al. が原典の文字・数字を逆に使っていると指摘
...
| **選定理由（毎件異なる切り口で）** | この論文が「なぜ今回の質問にとって面白いか」を、**前の論文と被らない切り口**で1〜2行で述べる（例：「実証データが具体的」「historical review として時代区分が秀逸」「Limitations 自認が誠実」「地域研究で意外な視座」「editorial として簡潔に総括」など、毎件異なる視点で） |
| 詳細抽出 | 主要B候補のみ：本文中の具体内容（ステップ5の出力） |
| 分類 | A / B1〜B7 / C / D |

**「選定理由」は毎件異なる切り口で書くこと**。単調な「重要文献である」「関連が高い」のような繰り返しは禁止。読み物として面白くなる粒度で。
...
## 3. 候補論文一覧表
（最低15件、B計7件以上、表形式・選定理由は毎件異なる切り口で）
...
- [ ] **「選定理由」を毎件異なる切り口で**書き、単調な繰り返しになっていないか
